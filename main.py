...
class Quiz:
    ...
    def buttons(self):
        next_button =next_button = Button(gui, text="Next", command=self.next_btn,width=10, bg="blue", fg="white", font=("times", 16, "bold"))
        # palcing the button on the screen
        next_button.place(x=200, y=380)
        # This is the second button which is used to Quit the GUI
        quit_button = Button(gui, text="Quit", command=gui.destroy,width=10, bg="green", fg="white", font=("times", 16, " bold"))
    ...
    def display_question(self):
        ...
        q_no = Label(gui, text=question[self.q_no], width=600, font=('ariel', 16, 'bold'))
        q_no.config(anchor="center")
        q_no.pack()

        # placing the option on the screen
        # q_no.place(x=0, y=100)

    # This method is used to Display Title

    def display_title(self):
        # The title to be shown
        title = Label(gui, text="HITAMO IGISUBIZO NYACYO",width=100, bg="Steelblue", fg="white", font=("ariel", 30, "bold"))
...
gui.title("IKIZAMINI CY'AMATEGEKO Y'UMUHANDA")
...
answer = (data['answer'])
zipped = zip(question,options,answer)
listed = list(zipped)
random.shuffle(listed)
question,options,answer= zip(*listed)
